// scribe-screens-2b.jsx — Project / Area dashboard (auto-composed).

function ProjectScreen() {
  const { t, go, Icon, Label, Tag, KIND } = window.bind();
  const { route } = window.useScribe();
  const pid = route.id || 'csp';
  const pname = window.projectName(pid) || 'Project';
  const area = window.areaOfProject(pid);
  const proj = area && area.projects.find((p) => p.id === pid);
  const owned = window.ownedNotes(pid);
  const linked = window.linkedMeetings(pid);
  const meetings = [
  ...
  const [briefing, setBriefing] = React.useState(pid === 'csp' ? 'ready' : 'idle'); // idle | running | ready
  ...
  const actions = window.actionsForProject(pid);
  ...
  return <div>
    ...
    <div style={{ padding: '20px 40px 0', maxWidth: 760, margin: '0 auto' }}>
      <form onSubmit={(e) => { e.preventDefault(); go({ screen: 'ask', query: askQ, scope: pname }); }}
        ...
        <span style={{ fontFamily: window.FONT, fontSize: 11, color: t.t3 }}>scoped</span>
      </form>
    </div>

    <div style={{ maxWidth: 760, margin: '0 auto', padding: '22px 40px 70px' }}>
      <div style={{ fontFamily: window.FONT, fontSize: 11.5, color: t.t3, marginBottom: 3 }}>Work · {area ? area.name : ''}</div>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap', marginBottom: 4 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <h1 style={{ fontFamily: window.FONT, fontSize: 28, fontWeight: 700, color: t.t1, letterSpacing: '-0.02em', margin: 0, whiteSpace: 'nowrap' }}>{pname}</h1>
          {proj && <window.StatusPill tone="accent">{proj.status}</window.StatusPill>}
        </div>
        <window.Btn kind="outline" size="sm" icon={briefing === 'running' ? 'loader-2' : 'refresh'} onClick={runBrief}>
          {briefing === 'ready' ? 'Refresh briefing' : briefing === 'running' ? 'Composing…' : 'Generate briefing'}</window.Btn>
      </div>
      <div style={{ fontFamily: window.FONT, fontSize: 11.5, color: t.t3, marginBottom: 20 }}>
      ...
      {briefing === 'ready' ? <div style={{ background: t.accentBg, border: '1px solid ' + t.accentLine, borderRadius: 12, padding: '15px 17px', marginBottom: 26 }}>
        ...
        <div style={{ fontFamily: window.FONT, fontSize: 14.5, color: t.t1, lineHeight: 1.65 }}>
          {window.BRIEFINGS[pid] || "Generate a briefing to see the current picture for this project."}
        </div>
      </div> : <div style={{ border: '1px dashed ' + t.line2, borderRadius: 12, padding: '18px 17px', marginBottom: 26,
        display: 'flex', alignItems: 'center', gap: 10, color: t.t3, fontFamily: window.FONT, fontSize: 13 }}>
        ...
        <div style={{ marginBottom: 26 }}>{actions.map((a, i) =>
          <div key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: 11, padding: '11px 6px', borderBottom: '1px solid ' + t.line }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div onClick={() => go({ screen: 'note', id: a.mid })} style={{ display: 'flex', alignItems: 'center', gap: 5, fontFamily: window.FONT, fontSize: 10.5, color: t.t3, marginTop: 3, cursor: 'pointer' }}>
          ...
          <div key={m.id} onClick={() => go({ screen: 'note', id: m.id })}
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                <span style={{ fontFamily: window.FONT, fontSize: 14, fontWeight: 600, color: t.t1, lineHeight: 1.3, whiteSpace: 'nowrap' }}>{m.title}</span>
                {lk && <span style={{ fontFamily: window.FONT, fontSize: 10, fontWeight: 600, color: t.t2, background: t.tagBg, borderRadius: 6, padding: '1px 7px', flex: 'none' }}>linked</span>}</div>
              <div style={{ fontFamily: window.FONT, fontSize: 11, color: t.t3, marginTop: 2 }}>
                {lk ? m.date + ' · homed in ' + window.areaName(m.area) + ' · mentions ' + pname : m.date + ' · ' + m.people.join(', ')}</div>
            </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 26 }}>{refs.map((r) =>
          <div key={r.id} onClick={() => go({ screen: 'note', id: r.id })}
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 7, marginBottom: 6 }}>
              <Icon n={KIND[r.kind].icon} s={14} c={t.t3} /><span style={{ fontFamily: window.FONT, fontSize: 10.5, color: t.t3 }}>{KIND[r.kind].label}</span></div>
            <div style={{ fontFamily: window.FONT, fontSize: 13.5, fontWeight: 600, color: t.t1, lineHeight: 1.3 }}>{r.title}</div>
          </div>)}</div>
      </>}
      ...
      {artifacts.length > 0 ? <div>
        {artifacts.map((d) => <div key={d.id} onClick={() => go({ screen: 'note', id: d.id })}
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontFamily: window.FONT, fontSize: 13.5, fontWeight: 600, color: t.t1 }}>{d.title}</div>
            <div style={{ fontFamily: window.FONT, fontSize: 11, color: t.t3, marginTop: 2 }}>Artifact · {d.date}</div>
          </div>
          <Icon n="arrow-up-right" s={14} c={t.t3} />
        </div>)}
        <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: 4 }}>
          <window.Btn kind="ghost" size="sm" icon="file-export" onClick={() => go({ screen: 'compose', projectId: pid })}>Compose another</window.Btn></div>
      </div> : <div style={{ border: '1px dashed ' + t.line2, borderRadius: 12, padding: '16px 17px', display: 'flex',
        alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
        <span style={{ fontFamily: window.FONT, fontSize: 13, color: t.t3 }}>Nothing composed yet.</span>
        <window.Btn kind="outline" size="sm" icon="file-export" onClick={() => go({ screen: 'compose', projectId: pid })}>Compose</window.Btn>
      </div>}
    </div>
  </div>;
}
